    ...
    <div className="space-y-4">
      ...
      <div className="widget overflow-hidden">
        <div className="px-4 py-3 border-b border-border/70">
          <div className="text-sm text-foreground">生成提供方</div>
          <div className="text-xs text-muted-foreground mt-0.5">选择默认使用的图像模型</div>
        </div>
        <div className="divide-y divide-border/70">
          {PROVIDERS.map((p) => (
            <button
              ...
              className="w-full flex items-center px-4 py-3 hover:bg-foreground/[0.04] text-left"
            >
              ...
              <div
                className={`w-4 h-4 rounded-full border ${
                  comicProvider === p.key ? "bg-amber-glow border-amber-glow" : "border-border"
                }`}
              />
            </button>
          ))}
      ...
      {comicProvider === "seedream" && (
        <div className="widget p-4 space-y-3">
          <div>
            <div className="text-sm text-foreground">Seedream 模型 ID</div>
          ...
          <input
            ...
            className="w-full bg-foreground/5 border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-amber-glow/50 font-mono"
          />
      ...
      <div className="widget p-4 space-y-2">
        ...
        <textarea
          ...
          rows={3}
          maxLength={500}
          className="w-full bg-foreground/5 border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-amber-glow/50 resize-none"
        />
        <div className="text-[10px] text-muted-foreground/70 text-right">{comicStyle.length}/500</div>
      </div>
      ...
      <div className="widget p-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm text-foreground">主角形象（可选）</div>
            <div className="text-xs text-muted-foreground mt-0.5">
        ...
        <div className="flex items-center gap-3">
          <div className="w-20 h-20 rounded-xl overflow-hidden bg-foreground/5 border border-border shrink-0 relative">
            {comicProtagonistUrl ? (
              <>
                <img
                ...
                <button
                  ...
                  className="absolute top-1 right-1 w-5 h-5 rounded-full bg-background/80 hover:bg-black/90 text-foreground flex items-center justify-center"
                  title="移除"
                >
  ...
  return (
    <div className="widget overflow-hidden">
      <div className="px-4 py-3 border-b border-border/70 flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm text-foreground">画面风格</div>
          <div className="text-xs text-muted-foreground mt-0.5">
          ...
          return (
            ...
            >
              {p.thumb ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-foreground/[0.05] text-muted-foreground/70 text-xs">
                  自由发挥
                </div>
              )}
      ...
      {hidden > 0 && !mustExpand && (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 py-2.5 border-t border-border/70 text-xs text-muted-foreground hover:text-foreground hover:bg-foreground/[0.04]"
        >
