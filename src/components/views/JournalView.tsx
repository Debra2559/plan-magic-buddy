...
export function JournalView({ embedded = false }: { embedded?: boolean } = {}) {
      ...
      <div className="h-full flex flex-col">
        <div className="flex items-center justify-between px-7 pt-5">
          <div>
            ...
            <h2 className="font-display text-2xl text-foreground">自由画布 · 拼贴你的日子</h2>
          </div>
  ...
  return (
    <div className="flex h-full">
        ...
        <div className="max-w-3xl mx-auto p-7">
          ...
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              className="p-2 rounded-lg hover:bg-foreground/10 text-muted-foreground"
              title="前一天"
            >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="bg-transparent text-xs text-foreground/85 outline-none border border-border rounded-md px-2 py-1"
              />
              {!isToday && (
                <button
                  onClick={() => setDate(todayLocal())}
                  className="text-[10px] px-2 py-1 rounded-full bg-foreground/5 border border-border text-muted-foreground hover:text-foreground"
                >
                  回到今天
                </button>
              )}
          ...
          <div className="relative rounded-[28px] overflow-hidden journal-paper">
              ...
              <div className="flex items-end justify-between mb-1">
                ...
                {dayDiary?.mood && (
                  <div className="text-right">
                    <p className="text-[10px] tracking-widest text-muted-foreground/70">心情</p>
                    <p className="text-3xl">{MOODS[dayDiary.mood]?.emoji}</p>
                    <p className="text-[10px] text-muted-foreground">{MOODS[dayDiary.mood]?.label}</p>
                  </div>
                )}
              </div>
              ...
              {dayItems.length > 0 && (
                <div className="mt-5 mb-7">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[10px] tracking-widest text-muted-foreground/70">今日完成率</span>
                    <span className="text-[10px] text-muted-foreground">{doneItems.length}/{dayItems.length} · {ratio}%</span>
                  </div>
                      ...
                      return (
                        <div key={it.id} className="relative">
                          ...
                          <div className="flex items-baseline gap-3">
                            ...
                            <p className={`text-sm leading-snug ${it.done ? "line-through text-muted-foreground/70" : "text-foreground"}`}>
                              {it.title}
                            </p>
                            <span className="ml-auto text-[10px] text-muted-foreground/70 shrink-0">{it.tag}</span>
                          </div>
                        </div>
                      );
                    })}
                    ...
                    return (
                      <button
                        ...
                        className={`group relative flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border transition active:scale-95 ${
                            ...
                            : "bg-foreground/5 border-dashed border-border text-muted-foreground/80 hover:border-amber-glow/40 hover:text-foreground/85"
                        }`}
            ...
            <div className="px-9 pb-7 pt-2 text-right">
              <p className="font-display text-sm text-muted-foreground/70 italic">— {aiName} ·  慢慢长出节奏 ·</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 右侧：过往 */}
      <aside className="w-60 shrink-0 bg-background/50 border-l border-border overflow-auto [scroll-padding-top:2.75rem]">
        <p className="sticky top-0 z-10 -mx-0 px-4 pt-4 pb-2 text-[10px] tracking-widest text-amber-glow bg-background/75 backdrop-blur-md border-b border-border/50">
          手帐翻页
        </p>
        <div className="p-4 pt-2">
          {allDates.length === 0 ? (
            <p className="text-xs text-muted-foreground/70">还没有任何记录</p>
          ) : (
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg transition border scroll-mt-12 ${
                      active
                        ? "bg-amber-glow/15 border-amber-glow/40 text-foreground"
                        : "bg-transparent border-border/70 text-muted-foreground hover:bg-foreground/5 hover:text-foreground"
                    }`}
                  >
                    <span
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground/70 mt-1">{f.sub}</p>
                    </span>
                  </button>
                );
              })}
...
function SectionHeader({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="flex items-center gap-2 mb-3 mt-1">
      ...
      <h3 className="text-xs tracking-[0.25em] text-foreground/75 uppercase">{title}</h3>
...
function ComicPanel({
  ...
  return (
    <div className="mb-7 rounded-2xl bg-foreground/[0.04] border border-border/70 p-4">
      ...
      {comic ? (
        <div className="space-y-2">
          <div className="relative rounded-xl overflow-hidden bg-background/60 border border-border/70">
            <img src={comic.imageUrl} alt={`${date} 漫画`} className="w-full h-auto block" />
            <button
              onClick={onRemove}
              className="absolute top-2 right-2 w-7 h-7 rounded-full bg-background/75 hover:bg-background/85 text-foreground/85 flex items-center justify-center"
              title="删除"
            >
          ...
          <div className="flex flex-wrap items-center gap-2 text-[10px] text-muted-foreground/80">
            <span className="px-2 py-0.5 rounded-full bg-foreground/5 border border-border">
            ...
            <button
              ...
              className="ml-auto flex items-center gap-1 px-2 py-0.5 rounded-full bg-foreground/5 hover:bg-foreground/10 border border-border text-foreground/75"
              title="导出为图片"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-foreground/5 hover:bg-amber-glow/20 border border-border hover:border-amber-glow/40 text-foreground/75 hover:text-amber-glow"
            >
          ...
          {comic.caption && (
            <p className="text-xs text-muted-foreground italic px-1">{comic.caption}</p>
          )}
        </div>
      ) : (
        <p className="text-xs text-muted-foreground/70 italic px-1 py-3">
...
function ComicHistoryPanel({
  ...
  if (history.length === 0) {
    return (
      <div className="mb-7 rounded-2xl bg-foreground/[0.03] border border-border/70 p-4">
        <p className="text-[11px] tracking-widest text-amber-glow/80 mb-2">Seedream 历史</p>
        <p className="text-xs text-muted-foreground/70 italic">还没有生成记录 —— 第一幅画即将诞生。</p>
      </div>
    );
  }
  ...
  return (
    <div className="mb-7 rounded-2xl bg-foreground/[0.04] border border-border/70 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <p className="text-[10px] text-muted-foreground/70">点击缩略图查看大图</p>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
        {history.map((item) => (
          <div
            key={item.id}
            className="group relative rounded-lg overflow-hidden border border-border bg-background/60 aspect-square"
          >
            <button
            ...
            <div className="absolute top-1 left-1 px-1.5 py-0.5 rounded-full bg-background/80 text-[9px] text-white/85 border border-border">
              {item.provider === "seedream" ? "Seedream" : "Gemini"}
            </div>
            <button
              onClick={(e) => { e.stopPropagation(); setConfirmId(item.id); }}
              className="absolute top-1 right-1 w-5 h-5 rounded-full bg-background/80 hover:bg-red-500/70 text-foreground/85 flex items-center justify-center opacity-0 group-hover:opacity-100 transition"
              title="删除"
            >
            ...
            <div className="absolute inset-x-0 bottom-0 px-1.5 py-1 bg-gradient-to-t from-black/85 to-transparent flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition">
              <div className="flex items-center justify-between text-[9px] text-foreground/75">
                <span>{item.date}</span>
              </div>
              <div className="flex gap-1">
                <button
                  onClick={() => onRecall(item)}
                  className="flex-1 px-1 py-0.5 rounded bg-amber-glow/30 hover:bg-amber-glow/50 text-[9px] text-foreground border border-amber-glow/40"
                ...
                <button
                  ...
                  className="flex-1 px-1 py-0.5 rounded bg-foreground/10 hover:bg-foreground/20 text-[9px] text-white/85 border border-border"
                  title="插入到今日日记"
                >
                  → 日记
                </button>
      ...
      {confirmId && (
        ...
        >
          <div
            className="bg-zinc-900 border border-border rounded-2xl p-5 max-w-xs w-full"
            onClick={(e) => e.stopPropagation()}
          >
            <p className="text-sm text-foreground mb-4">确定删除这张漫画历史吗？此操作不可撤销。</p>
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setConfirmId(null)}
                className="px-3 py-1.5 rounded-lg bg-foreground/10 hover:bg-foreground/15 text-xs text-foreground/85"
              >
                取消
              </button>
              <button
                onClick={() => { onDelete(confirmId); setConfirmId(null); }}
                className="px-3 py-1.5 rounded-lg bg-red-500/80 hover:bg-red-500 text-xs text-foreground"
              >
                删除
              </button>
            </div>
        ...
        >
          <div className="max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="relative rounded-2xl overflow-hidden border border-border bg-black">
              ...
              <button
                onClick={() => setPreview(null)}
                className="absolute top-2 right-2 w-8 h-8 rounded-full bg-background/80 hover:bg-black/90 text-foreground flex items-center justify-center"
              >
                <XIcon className="w-4 h-4" />
              </button>
            </div>
            <div className="flex flex-wrap items-center gap-2 mt-3 text-[11px] text-foreground/75">
              <span className="px-2 py-0.5 rounded-full bg-foreground/10 border border-border">
                {preview.provider === "seedream" ? "Seedream" : "Gemini"}
              </span>
              <span>原日期 · {preview.date}</span>
              <span className="text-muted-foreground/70">{new Date(preview.createdAt).toLocaleString()}</span>
              <button
                onClick={() => downloadComicImage(preview.imageUrl, `sylva-${preview.date}.png`)}
                className="ml-auto flex items-center gap-1 px-3 py-1 rounded-full bg-foreground/10 hover:bg-foreground/15 border border-border"
                title="导出为图片"
              >
                <Download className="w-3 h-3" /> 导出
              </button>
              ...
              <button
                ...
                className="px-3 py-1 rounded-full bg-foreground/10 hover:bg-foreground/15 border border-border"
              >
                插入到日记
              </button>
            </div>
            {preview.caption && (
              <p className="text-xs text-muted-foreground italic mt-2 px-1">{preview.caption}</p>
            )}
          </div>
        </div>
      )}
